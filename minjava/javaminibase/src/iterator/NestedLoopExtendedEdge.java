...
public class NestedLoopExtendedEdge extends Iterator {
	...
	public BTreeFile nodeIndexFile;
	public String realName;
	public ZTreeFile ndif;
	 ...
	 */
	public NestedLoopExtendedEdge(AttrType in1[], int len_in1, short t1_str_sizes[], AttrType in2[], int len_in2,
		...
		try {
			...
			if(relationName.equals("nodeLabelIndexFile"))
			{nodeIndexFile = db.nodeLabelIndexFile;}
			if(relationName.equals("nodeDescIndexFile"))
			{ndif = db.nodeDescriptorIndexFile;}
	...
	public Tuple get_next() throws IOException, JoinsException, IndexException, InvalidTupleSizeException,
		...
		do {
			...
			if (get_from_outer == true) {
				...
				try {
					StringKey low = new StringKey("");
					Descriptor desc = new Descriptor();
					desc.set(0, 0, 0, 0, 0);
					DescriptorKey hi = new DescriptorKey(desc);
					int source_or_dest = OutputFilter[0].operand1.symbol.offset;
					if(source_or_dest == 3){
						//System.out.println(outer_tuple.getIntFld(4));
						//System.out.println(outer_tuple.getIntFld(5));
						NID nid = new NID(new PageId(outer_tuple.getIntFld(4)),outer_tuple.getIntFld(5));
						Node node = db.nodeHeapfile.getNode(nid);
						if(realName.equals("nodeLabelIndexFile"))
						{low = new StringKey(node.getLabel());}
						if(realName.equals("nodeDescIndexFile"))
						{hi = new DescriptorKey(node.getDesc());}
						
						
					}
					if(source_or_dest == 1){
						NID nid = new NID(new PageId(outer_tuple.getIntFld(2)),outer_tuple.getIntFld(3));
						Node node = db.nodeHeapfile.getNode(nid);
						low = new StringKey(node.getLabel());
					}
					if(realName.equals("nodeLabelIndexFile"))
					{inner = nodeIndexFile.new_scan(low,low);}
					if(realName.equals("nodeDescIndexFile"))
					{inner = ndif.new_scan(hi,hi);}
...
//		        }
				...
				//System.out.println(outer_tuple.getStrFld(1));
				if (PredEval.Eval(RightFilter, null, node, null, _in2) == true) {
					//node.print();
					//if (outer_tuple.getStrFld(1).equals(labelofNode)) {
						// Apply a projection on the outer and inner tuples.
						Projection.Join(outer_tuple, _in1, node, _in2, Jtuple, perm_mat, nOutFlds);
						//get_from_outer = true;
						return Jtuple;
					//}
				}
