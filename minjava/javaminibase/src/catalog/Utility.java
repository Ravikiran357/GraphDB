...
public class Utility implements Catalogglobal {
	...
	public static void insertRecordUT(String relation, int attrCnt, attrNode[] attrList)
			throws Catalogmissparam, Catalogrelexists, Catalogdupattrs, Catalognomem, IOException, Catalogioerror,
			Cataloghferror, Catalogrelnotfound, Catalogindexnotfound, Catalogattrnotfound, Catalogbadattrcount,
			Catalogattrexists, Catalogbadtype, Exception {
	...
	static boolean check_float(attrNode N) {
		...
		if ((length > 1) && (index[count] == '.')) { // If we begin with a .,
														// then we must check to
														// make
		...
		// then we must check fror numbers, then check for ., then check for
		// numbers
		for (int i = count; i < length; i++) {
